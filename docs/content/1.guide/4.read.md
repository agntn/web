...
| Tavily | markdown, text | `format`, `timeout` | `TAVILY_API_KEY` |
| Exa | markdown | `timeout`, `noCache` | `EXA_API_KEY` |

`readProviders()` lists them at runtime, custom readers included, `isReadProvider(provider)` is the type guard.

`maxTokens` is Jina's token budget, and it's a tripwire, not scissors. A page over it doesn't come back cut, Jina refuses it with a 409. Firecrawl rejects `maxTokens` on an explicit read instead of ignoring it. You asked for a token bound and got an unbounded page, that is a lie, not a feature.
...
A reader that just ran out of credits waits at the back of the chain for 30 minutes and shows up in `skipped`, the same as in [automatic search](/guide/fanout).

A `maxTokens` meant for Jina doesn't follow the chain. Automatic reads leave it out for every reader that doesn't list it. So Firecrawl reads the page instead of refusing the option, and the detailed answer says so with `ignoredOptions: ["maxTokens"]`. A page Jina refused over budget falls through the same way. The next reader returns it whole, so pass `maxChars` when the size matters. A custom reader that declares no options still gets `maxTokens`.
