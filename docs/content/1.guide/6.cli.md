...
A failure that ends the command, a spent key, a bad token, a date that is not ISO 8601, is one line on stderr and exit code 1. Nothing lands on stdout, `--json` included.
