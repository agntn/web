...
A 401 normally becomes `AuthError`. Context.dev's `USAGE_EXCEEDED` is the exception: spent credits are not a bad key. It becomes `PaymentError`, an `HTTPError` subclass that preserves the original 401, redacted URL and body and allows automatic fallback. A 429 becomes `RateLimitError` with `retryAfter`; other failures use `HTTPError` or `WebError`. All of them extend `WebError`, one `instanceof` catches the lot.
