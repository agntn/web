...
Importing `@agntn/web` registers the twelve built-in providers as a side effect. After that `create("brave")`, `create("tavily")` or `create("searxng")` resolves the class, no switch statement anywhere. The key comes from the provider's env var unless you pass `apiKey` yourself:
...
Codex search uses an explicit OAuth token and account ID rather than an API key. See [OpenAI Codex](/providers/openai-codex) for environment setup and a credential callback that can refresh a login.
...
| OpenAI Codex | `OPENAI_CODEX_ACCESS_TOKEN` + `OPENAI_CODEX_ACCOUNT_ID` | yes | | |
