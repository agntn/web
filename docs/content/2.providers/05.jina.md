...
const jina = await create("jina"); // JINA_API_KEY, required for search
...
Search results carry the page content in `text`, plus `publishedDate`, `image` and `metadata`. `includeDomains` and a `category` of `web`, `images` or `news` work. Reads give `markdown`, `text` or `html` and take `maxTokens` natively, as a budget: a longer page fails with a 409 instead of coming back cut.
