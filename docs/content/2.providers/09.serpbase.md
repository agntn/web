...
const serpbase = await create("serpbase"); // SERPBASE_API_KEY
...
- Credits are charged per request and reported in `metadata.creditsCharged`. An empty balance answers HTTP 200 with `status: 1020`. That is `PaymentError` here, so automatic search tries the next configured provider instead of stopping.
