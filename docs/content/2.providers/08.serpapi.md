...
- `maxResults` is applied on our side to whatever Google or Lens returns. Google pays little attention to `num` (nothing below ten, and since 2025 not much above it), so a page is ten organic results and `start` still moves by ten: `maxResults: 3` gives you three of each ten.
