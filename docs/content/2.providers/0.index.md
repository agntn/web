...
navigation:
  title: Overview
description: Thirteen providers, one class shape. Pick one to see its endpoints, the filters it honours, the fields it fills and the traps.
---

Every search provider answers the same `search()` with the same `SearchResult`. Six of them also `read()`, one also `searchByImage()`. What changes is the API behind it, the filters it can honour, which optional fields it fills, and what ends up in `metadata`. The pages are short on purpose. The part worth reading on each one is the gotcha list.

| Provider | Env var | Search | Image | Read | Filters | Paging |
| --- | --- | --- | --- | --- | --- | --- |
| [Brave](/providers/brave) | `BRAVE_API_KEY` | yes | | | dates | yes |
| [Context.dev](/providers/context) | `CONTEXT_DEV_API_KEY` | yes | | yes | domains | |
| [Exa](/providers/exa) | `EXA_API_KEY` | yes | | yes | domains, category, dates | |
...
| [Marginalia](/providers/marginalia) | `MARGINALIA_API_KEY`, or the shared `public` key when named | yes | | | none | yes |
| [Mojeek](/providers/mojeek) | `MOJEEK_API_KEY` | yes | | | domains, dates | yes |
| [OpenAI Codex](/providers/openai-codex) | Existing login; optional `OPENAI_CODEX_ACCESS_TOKEN` | yes | | | none | |
| [SearXNG](/providers/searxng) | none | yes | | | category | yes |
| [SerpAPI](/providers/serpapi) | `SERPAPI_API_KEY` | yes | yes | | dates | yes |
| [SerpBase](/providers/serpbase) | `SERPBASE_API_KEY` | yes | | | category | yes |
| [Tavily](/providers/tavily) | `TAVILY_API_KEY` | yes | | yes | domains, category, dates | |
