---
title: Providers
icon: i-lucide-library
navigation:
  title: Overview
description: Twelve providers, one class shape. Pick one to see its endpoints, the filters it honours, the fields it fills and the traps.
---

Every search provider answers the same `search()` with the same `SearchResult`. Four of them also `read()`, one also `searchByImage()`. What changes is the API behind it, the filters it can honour, which optional fields it fills, and what ends up in `metadata`. The pages are short on purpose. The part worth reading on each one is the gotcha list.

| Provider | Env var | Search | Image | Read | Filters | Paging |
| --- | --- | --- | --- | --- | --- | --- |
| [Brave](/providers/brave) | `BRAVE_API_KEY` | yes | | | none | yes |
| [Context.dev](/providers/context) | `CONTEXT_DEV_API_KEY` | yes | | yes | domains | |
| [Exa](/providers/exa) | `EXA_API_KEY` | yes | | | domains, category, dates | |
| [Firecrawl](/providers/firecrawl) | `FIRECRAWL_API_KEY` | yes | | yes | domains, sources, categories | |
| [Jina](/providers/jina) | `JINA_API_KEY` | yes | | yes | include domains, category | |
| [Mojeek](/providers/mojeek) | `MOJEEK_API_KEY` | yes | | | domains, dates | yes |
| [OpenAI Codex](/providers/openai-codex) | `OPENAI_CODEX_ACCESS_TOKEN` + `OPENAI_CODEX_ACCOUNT_ID` | yes | | | none | |
| [SearXNG](/providers/searxng) | none | yes | | | category | yes |
| [SerpAPI](/providers/serpapi) | `SERPAPI_API_KEY` | yes | yes | | none | yes |
| [SerpBase](/providers/serpbase) | `SERPBASE_API_KEY` | yes | | | category | yes |
| [Tavily](/providers/tavily) | `TAVILY_API_KEY` | yes | | | domains | |
| [TinyFish](/providers/tinyfish) | `TINYFISH_API_KEY` | yes | | yes | domains, category, dates | yes |

:::card-group
  ::card
  ---
  icon: i-simple-icons-brave
  to: /providers/brave
  title: Brave
  ---
  The Brave Search API, one GET per page, with extra snippets joined into text and a continuation that carries the offset.
  ::

  ::card
  ---
  icon: i-lucide-book-text
  to: /providers/context
  title: Context.dev
  ---
  The Context.dev API for search with Markdown ranked by relevance and for scraping pages into Markdown or HTML.
  ::

  ::card
  ---
  icon: i-lucide-sparkles
  to: /providers/exa
  title: Exa
  ---
  The Exa neural search API, the richest result shape of the built-ins, with highlights by default and summaries and full text on request.
  ::

  ::card
  ---
  icon: i-lucide-flame
  to: /providers/firecrawl
  title: Firecrawl
  ---
  The Firecrawl v2 API for search with scraped passages and for scraping a page into Markdown or HTML.
  ::

  ::card
  ---
  icon: i-lucide-file-text
  to: /providers/jina
  title: Jina
  ---
  Jina's s.jina.ai for search and r.jina.ai for reading, the default reader, with an optional key.
  ::

  ::card
  ---
  icon: i-simple-icons-mojeek
  to: /providers/mojeek
  title: Mojeek
  ---
  The Mojeek Search API, an independent index with classic SERP metadata, domain and date filters and paging.
  ::

  ::card
  ---
  icon: i-lucide-search
  to: /providers/openai-codex
  title: OpenAI Codex
  ---
  Hosted web search through the ChatGPT Codex backend with explicit OAuth credentials. Native sources, not snippets invented by the model.
  ::

  ::card
  ---
  icon: i-simple-icons-searxng
  to: /providers/searxng
  title: SearXNG
  ---
  A self-hosted SearXNG metasearch instance, no key, with a reachability probe and the engines behind each result in metadata.
  ::

  ::card
  ---
  icon: i-lucide-scan-search
  to: /providers/serpapi
  title: SerpAPI
  ---
  SerpAPI's Google endpoints for classic SERP results and Google Lens for reverse image search.
  ::

  ::card
  ---
  icon: i-lucide-layers
  to: /providers/serpbase
  title: SerpBase
  ---
  SerpBase's Google SERP endpoints, with a category that picks the web, images, news or videos endpoint.
  ::

  ::card
  ---
  icon: i-lucide-compass
  to: /providers/tavily
  title: Tavily
  ---
  The Tavily search API, with an optional generated answer in response metadata and raw page content on request.
  ::

  ::card
  ---
  icon: i-lucide-fish
  to: /providers/tinyfish
  title: TinyFish
  ---
  TinyFish Search for news and research results with publisher metadata, and TinyFish Fetch for reading pages.
  ::
:::
